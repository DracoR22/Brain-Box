...
const TrashRestore = () => {
    ...
    useEffect(() => {
      const stateFolders = state.workspaces.find((workspace) => workspace.id === workspaceId)
    ...
    <section>
        {!!folders.length && (
            <>
              ...
              {folders.map((folder) => (
                 <Link href={`/dashboard/${folder.workspaceId}/${folder.id}`} key={folder.id}
        ...
        {!!files?.length && (
            <>
              ...
              {files.map((file) => (
                <Link href={`/dashboard/${file.workspaceId}/${file.folderId}/${file.id}`} key={file.id}
                className="hover:bg-muted rounded-md p-2 flex items-center justify-between">
                   <article>
                       <aside className="flex items-center gap-2">
                           <FileIcon/>
